...
import '../EditAd/EditAdScreenDeskTop.dart';

class AdminAdsDesktopView extends StatefulWidget {
  const AdminAdsDesktopView({Key? key}) : super(key: key);

  @override
  _AdminAdsDesktopViewState createState() => _AdminAdsDesktopViewState();
}

class _AdminAdsDesktopViewState extends State<AdminAdsDesktopView> {
...
// ======= في initState() بعد استدعاء _loadInitialData() =======
@override
void initState() {
  super.initState();
  _loadInitialData().then((_) {
    // ضبط القيمة الابتدائية للعرض بناءً على أول إعلان (إن وجد)
    _optimisticShowTime.value = controller.adminAdsList.isNotEmpty
        ? controller.adminAdsList.first.show_time == 1
        : false;
  });
}
  ...
  Future<void> _loadInitialData() async {
    ...
    await controller.fetchAdminAds(lang: _lang, status: 'published');
  }
  ...
  void _clearFilters() {
    ...
    controller.fetchAdminAds(lang: _lang, status: 'published');
  }
      ...
      AlertDialog(
        ...
        title: Row(
          ...
          children: [
            ...
            SizedBox(width:10.w),
            Text('إلغاء نشر الاعلان', style: TextStyle(
              fontSize:15.sp, fontFamily: AppTextStyles.tajawal,
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('هل انت متاكد بالغاء  نشر الإعلان?', style: TextStyle(
              fontSize:13.sp, fontFamily: AppTextStyles.tajawal,
              color: AppColors.textPrimary(isDark)),
            ),
            SizedBox(height: 8.h),
            ...
            children: [
              ...
              Obx(() => ElevatedButton(
                style: ElevatedButton.styleFrom(
                  ...
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12.r))),
                onPressed: controller.isLoadingAdminAds.value ? null : () {
                  controller.togglePublish(adId,'published','غير منشور');
                  Get.back();
                },
                child: controller.isLoadingAdminAds.value
                  ? SizedBox(
  ...
  // متحكمات النصوص
  TextEditingController slugController = TextEditingController(text: ad.slug ?? '');
  TextEditingController metaTitleController = TextEditingController(text: ad.meta_title ?? '');
  TextEditingController metaDescriptionController = TextEditingController(text: ad.meta_description ?? '');
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSeoField('Slug (رابط الإعلان)', slugController, Icons.link),
            ...
            Row(
              children: [
                ...
                Obx(() => ElevatedButton(
                  onPressed: controller.isUpdatingAd.value ? null : () async {
                    await controller.updateAdSeo(
                      ad.id,
                  ...
                  child: controller.isUpdatingAd.value
                      ? SizedBox(
                          width: 20.r,
                          height: 20.r,
                          child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2.r),
                        )
                      : Text('حفظ', style: TextStyle(fontSize: 14.sp)),
                )),
              ],
            ),
          ],
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        children: [
                          ...
                          Wrap(
                            ...
                            children: [
                              ...
                              _buildInfoChip('عرض الوقت: ${ad.show_time == 1 ? 'مرئي' : 'مخفي'}', Icons.access_time),
                            ],
                          ),
                        ],
                      ),
                    ...
                    // بيانات SEO - جديد
                    if (ad.slug != null || ad.meta_title != null || ad.meta_description != null) ...[
                      Container(
                        padding: EdgeInsets.all(16.r),
                        decoration: BoxDecoration(
                          color: AppColors.surface(isDark),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 12.h),
                            if (ad.slug != null) _buildSeoInfoRow('Slug:', ad.slug!),
                            if (ad.meta_title != null) _buildSeoInfoRow('Meta Title:', ad.meta_title!),
                            if (ad.meta_description != null) _buildSeoInfoRow('Meta Description:', ad.meta_description!),
                          ],
                        ),
                      ),
                      SizedBox(height: 20.h),
...
// ويدجت لعرض معلومات SEO
Widget _buildSeoInfoRow(String title, String value) {
  final isDark = Theme.of(context).brightness == Brightness.dark;
  final textColor = AppColors.textPrimary(isDark);
        ...
        Expanded(child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, textDirection: TextDirection.rtl, children: [
            ...
            Row(textDirection: TextDirection.rtl, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('إدارة الإعلانات', style: TextStyle(
            ...
            // زر تبديل عرض الوقت
          // ======= استبدل قسم "زر تبديل عرض الوقت" بالكود التالي =======
Obx(() {
  // حالة التحميل من الكنترولر
  final isLoading = controller.isTogglingShowTime.value;

  // الحالة التي نعرضها للـ UI (نستخدم الحالة المتفائلة أولاً ثم نعود لبيانات الكنترولر عند الحاجة)
  final visible = _optimisticShowTime.value ||
      (controller.adminAdsList.isNotEmpty && controller.adminAdsList.first.show_time == 1);
  ...
  return Container(
    ...
    child: ElevatedButton.icon(
      onPressed: isLoading
          ? null
          : () async {
              // حساب القيمة المراد تبديلها (العكس)
              final current = controller.adminAdsList.isNotEmpty
                  ? controller.adminAdsList.first.show_time == 1
                  : _optimisticShowTime.value;
              final intendedNew = !current;

              // تحديث واجهة المستخدم فوراً (optimistic)
              _optimisticShowTime.value = intendedNew;

              try {
                // نداء API للتبديل (الكنترولر يتولى إجراء الطلب)
                await controller.toggleShowTimeGlobal();

                // بعد نجاح الطلب نعيد جلب الإعلانات لتأكيد الحالة الحقيقية
                await controller.fetchAdminAds(lang: _lang, status: 'published');

                // اضبط الحالة المتفائلة حسب البيانات الحقيقية
                _optimisticShowTime.value = controller.adminAdsList.isNotEmpty
                    ? controller.adminAdsList.first.show_time == 1
                    : intendedNew;
              } catch (e) {
                // إذا صار خطأ - نعيد ضبط الحالة إلى ما قبل الضغط (أو نقرأ الحالة الحقيقية)
                _optimisticShowTime.value = controller.adminAdsList.isNotEmpty
                    ? controller.adminAdsList.first.show_time == 1
                    : !intendedNew;
                // عرض رسالة خطأ اختياري (الكنترولر قد يعرضها بالفعل)
              }
            },
      style: ElevatedButton.styleFrom(
        backgroundColor: visible ? AppColors.success : AppColors.warning,
        foregroundColor: Colors.white,
      ...
      label: Text(
        visible ? 'إخفاء الوقت (الحالي: مرئي)' : 'إظهار الوقت (الحالي: مخفي)',
        style: TextStyle(
          fontSize: 14.sp,
          fontFamily: AppTextStyles.tajawal,
          fontWeight: FontWeight.bold,
                ...
                child: CustomScrollView(slivers: [
                  SliverToBoxAdapter(child: Container(
                    ...
                    child: Row(textDirection: TextDirection.rtl, children: [
                      ...
                      _buildHeaderCell("إعدادات SEO", 1), // عمود جديد
...
Widget _buildAdRow(Ad ad, int index, bool isDark, Color color) {
  ...
  return Container(
    ...
    child: Row(textDirection: TextDirection.rtl, children: [
      ...
      _buildCell(_formatPrice(ad.price), 1),
      ...
      Expanded(
        ...
        child: Row(
          ...
          children: [
            ...
            // زر التعديل
            IconButton(
              icon: Icon(Icons.edit, size: 18.r, color: AppColors.warning),
              onPressed: () {
